...
FILE *fp;
if (argc<2) {
  printf("./paste_ml_format_4smiles.exe Protein-Ligand_Parameter\n");
  exit(1);
}
int hbd=0, hba=0, heavy_atom=0;
float logp=0, mr=0, wiener=0, molW=0, expt_be=0;
char st[500];
char pdbid[10];
...
while(fgets(st,500,fp)!=NULL)
	{
	sscanf(st,"%s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %d %f %f %f %f", pdbid, &pd_charged, &pa_charged, &pd_oh, &pa_O, &pd_nh, &pa_lyn, &pd_arom, &pa_arom, &pd_amide, &pa_amide, &pp_ali, &pp_arom, &mr_ali, &mr_arom, &volm, &hbd, &hba, &logp, &mr, &wiener, &molW);
