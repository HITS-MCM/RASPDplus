 ...
 * Purpose: This code print the physicochemical parameters seperated by ";"
...
FILE *fp;
if (argc<2) {
  printf("./paste.exe Protein-Ligand_Parameter\n");
  exit(1);
}
char st[500];
char pdbid[5]="zinc";
...
float logp=0, mr=0, wiener=0, rotbond=0, molW=0, expt_be=0, atom_effi=0;
int hbd=0, hba=0, noofatoms=0;
fp=fopen(argv[1],"r");

printf("PDBID;PD(K+R+HIP);PA(D+E);PD(T+S+Y+DH+EH);PA(N+Q+T+S+DH+EH);PD(LYN+N+Q);PA(LYN);PD(W+H);PA(Y+H);PD(Amide-NH);PA(Amide-O);PP(Non-Arom);PP(Arom);PMR(Non-Arom);PMR(Arom);Volume;D;A;P;MR;W;rot_bonds;MASS;Expt_BE;Atom_Efficiency;No. of atoms\n");
...
printf("%s;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%0.3f;%d;%d;%0.3f;%0.3f;%0.3f;%0.3f;%0.2f;%0.3f;%0.6f;%d\n", pdbid, pd_charged, pa_charged, pd_oh, pa_O, pd_nh, pa_lyn, pd_arom, pa_arom, pd_amide, pa_amide, pp_ali, pp_arom, mr_ali, mr_arom, volm, hbd, hba, logp, mr, wiener, rotbond, molW, expt_be, atom_effi, noofatoms);
